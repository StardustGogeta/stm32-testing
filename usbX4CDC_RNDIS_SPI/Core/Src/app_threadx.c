...
void MainThread_Entry(unsigned long thread_input)
{
	...
	while (1)
	{
		...
		HAL_SPI_Receive(&hspi1, buf, sizeof(buf), 500);
		int x = strcmp((char*)buf, "Hello Arduino world please work");
		HAL_UART_Transmit(&huart2, "Str::", 6, 1000);
		HAL_UART_Transmit(&huart2, (unsigned char*)buf, strlen(buf) + 1, 1000);
